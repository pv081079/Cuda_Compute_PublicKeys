...
__global__ void generate_keypair_kernel(curandState* state, uint32_t* prvKeys, uint32_t* compressedPubKeys, uint32_t* uncompressedPubKeys) {
    ...
    private_to_public_full(prvKeys, uncompressedPubKeys);
...
int main() {
    ...
    cudaMalloc((void**)&d_uncompressedPubKeys, 16 * sizeof(uint32_t)); // 8 for x, 8 for y
    ...
    uint32_t h_uncompressedPubKeys[16];
    ...
    cudaMemcpy(h_uncompressedPubKeys, d_uncompressedPubKeys, 16 * sizeof(uint32_t), cudaMemcpyDeviceToHost);
    cudaCheckError();
    ...
    // Convert the compressed public key to a uint8_t array with byte order correction
    uint8_t pub[33];
    memcpy(pub, (uint8_t*)h_compressedPubKeys, 33);

    // Print the compressed public key with corrected byte order
    ...
    // Print the uncompressed public key with corrected byte order
    printf("Uncompressed Public Key: 04");
    for (int i = 0; i < 16; i++) {
        uint32_t reversed = reverse_bytes(h_uncompressedPubKeys[i]);
        printf("%02x%02x%02x%02x", 
            (reversed >> 24) & 0xFF, 
            (reversed >> 16) & 0xFF, 
            (reversed >> 8) & 0xFF, 
